...
|====
|Application |Build tools (internet computer) |Runtime dependencies |Standalone-admin |Standalone-user |Portable-R/W (6) |Portable-R/O (7)
...
(2) This is the simplest way to get asciidoctor running, especially if you already use Ant.
However asciidoctor-ant only includes the main asciidoctor gem, and is often a version behind.

(3) There is no pre-built version available, and the project does not build off-line.
...
(7) Eg CD or read-only network drive.

(8) If Ruby is not on the target computer, JRuby can be used instead (without admin install) but it needs a JVM.

=== Build tools

Installation of the build tools (Ruby, JDK, and Maven) on the internet computer are not described here because they are already fully described elsewhere.
...
Follow the intsructions for portable R/W as above.
